...
def build_agent():
    ...
    if not IS_ACTIVE:
        raise RuntimeError(
            "drawing_approval_orchestrator is DORMANT. Set is_active=true in Config Registry to activate."
        )
    # engine = resolve_engine("basco", "drawing-approval")
    # return LlmAgent(
    #     name="drawing_approval_orchestrator",
    #     model=engine["model"],
...
def run_stub(payload: dict) -> dict:
    ...
    result = {
        "agent": "drawing_approval_orchestrator",
        "status": "NOOP",
