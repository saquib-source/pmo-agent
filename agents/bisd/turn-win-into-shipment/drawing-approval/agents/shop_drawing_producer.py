...
def build_agent():
    ...
    if not IS_ACTIVE:
        raise RuntimeError(
            "shop_drawing_producer_agent is DORMANT. Set is_active=true in Config Registry to activate."
        )
    # engine = resolve_engine("basco", "drawing-approval")
    # return LlmAgent(
    #     name="shop_drawing_producer_agent",
    #     model=engine["model"],
    #     instruction=open("prompts/shop-drawing-producer.md").read(),
    # )
...
def run_stub(payload: dict) -> dict:
    ...
    result = {
        "agent": "shop_drawing_producer_agent",
        "status": "NOOP",
