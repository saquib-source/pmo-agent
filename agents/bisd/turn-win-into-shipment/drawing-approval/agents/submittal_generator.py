...
def build_agent():
    ...
    if not IS_ACTIVE:
        raise RuntimeError(
            "submittal_generator_agent is DORMANT. Set is_active=true in Config Registry to activate."
        )
    # engine = resolve_engine("basco", "drawing-approval")
    # return LlmAgent(
    #     name="submittal_generator_agent",
    #     model=engine["model"],
    #     instruction=open("prompts/submittal-generator.md").read(),
    # )
...
def run_stub(payload: dict) -> dict:
    ...
    result = {
        "agent": "submittal_generator_agent",
        "status": "NOOP",
