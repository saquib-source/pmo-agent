"""
Order Realization Orchestrator
...
def build_agent():
    ...
    if not IS_ACTIVE:
        raise RuntimeError(
            "order_realization_orchestrator is DORMANT. Set is_active=true in Config Registry to activate."
        )
    # engine = resolve_engine("basco", "order-realization")
    # return LlmAgent(
    #     name="order_realization_orchestrator",
    #     model=engine["model"],
...
def run_stub(payload: dict) -> dict:
    ...
    result = {
        "agent": "order_realization_orchestrator",
        "status": "NOOP",
