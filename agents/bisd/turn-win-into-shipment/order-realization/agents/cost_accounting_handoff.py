...
def build_agent():
    ...
    if not IS_ACTIVE:
        raise RuntimeError(
            "cost_accounting_handoff_agent is DORMANT. Set is_active=true in Config Registry to activate."
        )
    # engine = resolve_engine("basco", "order-realization")
    # return LlmAgent(
    #     name="cost_accounting_handoff_agent",
    #     model=engine["model"],
    #     instruction=open("prompts/cost-accounting-handoff.md").read(),
    # )
...
def run_stub(payload: dict) -> dict:
    ...
    result = {
        "agent": "cost_accounting_handoff_agent",
        "status": "NOOP",
