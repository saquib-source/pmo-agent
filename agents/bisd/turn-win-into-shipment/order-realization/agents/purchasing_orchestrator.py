"""
Purchasing Orchestrator
BISD swarm: order-realization
...
def build_agent():
    ...
    if not IS_ACTIVE:
        raise RuntimeError(
            "purchasing_orchestrator_agent is DORMANT. Set is_active=true in Config Registry to activate."
        )
    # engine = resolve_engine("basco", "order-realization")
    # return LlmAgent(
    #     name="purchasing_orchestrator_agent",
    #     model=engine["model"],
    #     instruction=open("prompts/purchasing-orchestrator.md").read(),
    # )
...
def run_stub(payload: dict) -> dict:
    ...
    result = {
        "agent": "purchasing_orchestrator_agent",
        "status": "NOOP",
        "reason": "STUB — agent is_active=false",
        "payload_received": bool(payload),
    }
    # log_decision(agent_id="order-realization", action="stub_noop", outcome="noop")
    return result
