...
def build_agent():
    ...
    if not IS_ACTIVE:
        raise RuntimeError(
            "warehouse_coordinator_agent is DORMANT. Set is_active=true in Config Registry to activate."
        )
    # engine = resolve_engine("basco", "order-realization")
    # return LlmAgent(
    #     name="warehouse_coordinator_agent",
    #     model=engine["model"],
    #     instruction=open("prompts/warehouse-coordinator.md").read(),
    # )
