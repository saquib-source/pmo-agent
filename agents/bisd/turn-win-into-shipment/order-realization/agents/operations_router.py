...
def build_agent():
    ...
    if not IS_ACTIVE:
        raise RuntimeError(
            "operations_router_agent is DORMANT. Set is_active=true in Config Registry to activate."
        )
    # engine = resolve_engine("basco", "order-realization")
    # return LlmAgent(
    #     name="operations_router_agent",
    #     model=engine["model"],
    #     instruction=open("prompts/operations-router.md").read(),
    # )
