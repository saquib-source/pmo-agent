...
def build_agent():
    ...
    if not IS_ACTIVE:
        raise RuntimeError(
            "field_measurement_agent is DORMANT. Set is_active=true in Config Registry to activate."
        )
    # engine = resolve_engine("basco", "field-measurement")
    # return LlmAgent(
    #     name="field_measurement_agent",
    #     model=engine["model"],
...
def run_stub(payload: dict) -> dict:
    ...
    result = {
        "agent": "field_measurement_agent",
        "status": "NOOP",
        "reason": "STUB — agent is_active=false",
        "payload_received": bool(payload),
    }
    # log_decision(agent_id="field-measurement", action="stub_noop", outcome="noop")
    return result
