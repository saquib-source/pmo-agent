...
def build_agent():
    ...
    if not IS_ACTIVE:
        raise RuntimeError(
            "staging_coordinator_agent is DORMANT. Set is_active=true in Config Registry to activate."
        )
    # engine = resolve_engine("basco", "consolidated-shipment")
    # return LlmAgent(
    #     name="staging_coordinator_agent",
    #     model=engine["model"],
    #     instruction=open("prompts/staging-coordinator.md").read(),
    # )
...
def run_stub(payload: dict) -> dict:
    ...
    result = {
        "agent": "staging_coordinator_agent",
        "status": "NOOP",
