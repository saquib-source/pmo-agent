...
def build_agent():
    ...
    if not IS_ACTIVE:
        raise RuntimeError(
            "cross_dock_agent_agent is DORMANT. Set is_active=true in Config Registry to activate."
        )
    # engine = resolve_engine("basco", "consolidated-shipment")
    # return LlmAgent(
    #     name="cross_dock_agent_agent",
    #     model=engine["model"],
    #     instruction=open("prompts/cross-dock-agent.md").read(),
    # )
