"""
Consolidated Shipment Orchestrator
BISD swarm: consolidated-shipment
...
def build_agent():
    ...
    if not IS_ACTIVE:
        raise RuntimeError(
            "consolidated_shipment_orchestrator is DORMANT. Set is_active=true in Config Registry to activate."
        )
    # engine = resolve_engine("basco", "consolidated-shipment")
    # return LlmAgent(
    #     name="consolidated_shipment_orchestrator",
    #     model=engine["model"],
...
def run_stub(payload: dict) -> dict:
    ...
    result = {
        "agent": "consolidated_shipment_orchestrator",
        "status": "NOOP",
        "reason": "STUB — agent is_active=false",
        "payload_received": bool(payload),
    }
    # log_decision(agent_id="consolidated-shipment", action="stub_noop", outcome="noop")
    return result
