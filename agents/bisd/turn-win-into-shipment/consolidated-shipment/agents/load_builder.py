...
def build_agent():
    ...
    if not IS_ACTIVE:
        raise RuntimeError(
            "load_builder_agent is DORMANT. Set is_active=true in Config Registry to activate."
        )
    # engine = resolve_engine("basco", "consolidated-shipment")
    # return LlmAgent(
    #     name="load_builder_agent",
    #     model=engine["model"],
    #     instruction=open("prompts/load-builder.md").read(),
    # )
