...
def build_agent():
    ...
    if not IS_ACTIVE:
        raise RuntimeError(
            "listings_manager_agent is DORMANT. Set is_active=true in Config Registry to activate."
        )
    # engine = resolve_engine("basco", "findability")
    # return LlmAgent(
    #     name="listings_manager_agent",
    #     model=engine["model"],
    #     instruction=open("prompts/listings-manager.md").read(),
    # )
