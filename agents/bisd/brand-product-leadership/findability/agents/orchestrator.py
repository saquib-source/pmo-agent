...
def build_agent():
    ...
    if not IS_ACTIVE:
        raise RuntimeError(
            "findability_orchestrator is DORMANT. Set is_active=true in Config Registry to activate."
        )
    # engine = resolve_engine("basco", "findability")
    # return LlmAgent(
    #     name="findability_orchestrator",
    #     model=engine["model"],
...
def run_stub(payload: dict) -> dict:
    ...
    result = {
        "agent": "findability_orchestrator",
        "status": "NOOP",
