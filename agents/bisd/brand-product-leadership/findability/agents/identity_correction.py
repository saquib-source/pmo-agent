"""
Identity Correction Agent
BISD swarm: findability
...
def build_agent():
    ...
    if not IS_ACTIVE:
        raise RuntimeError(
            "identity_correction_agent is DORMANT. Set is_active=true in Config Registry to activate."
        )
    # engine = resolve_engine("basco", "findability")
    # return LlmAgent(
    #     name="identity_correction_agent",
    #     model=engine["model"],
    #     instruction=open("prompts/identity-correction.md").read(),
    # )
...
def run_stub(payload: dict) -> dict:
    ...
    result = {
        "agent": "identity_correction_agent",
        "status": "NOOP",
        "reason": "STUB — agent is_active=false",
        "payload_received": bool(payload),
    }
    # log_decision(agent_id="findability", action="stub_noop", outcome="noop")
    return result
