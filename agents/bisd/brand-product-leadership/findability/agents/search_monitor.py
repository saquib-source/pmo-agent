...
def build_agent():
    ...
    if not IS_ACTIVE:
        raise RuntimeError(
            "search_monitor_agent is DORMANT. Set is_active=true in Config Registry to activate."
        )
    # engine = resolve_engine("basco", "findability")
    # return LlmAgent(
    #     name="search_monitor_agent",
    #     model=engine["model"],
    #     instruction=open("prompts/search-monitor.md").read(),
    # )
