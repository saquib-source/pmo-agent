"""
Reputation Management Orchestrator
BISD swarm: reputation-management
...
def build_agent():
    ...
    if not IS_ACTIVE:
        raise RuntimeError(
            "reputation_management_orchestrator is DORMANT. Set is_active=true in Config Registry to activate."
        )
    # engine = resolve_engine("basco", "reputation-management")
    # return LlmAgent(
    #     name="reputation_management_orchestrator",
    #     model=engine["model"],
...
def run_stub(payload: dict) -> dict:
    ...
    result = {
        "agent": "reputation_management_orchestrator",
        "status": "NOOP",
        "reason": "STUB — agent is_active=false",
        "payload_received": bool(payload),
    }
    # log_decision(agent_id="reputation-management", action="stub_noop", outcome="noop")
    return result
