...
def build_agent():
    ...
    if not IS_ACTIVE:
        raise RuntimeError(
            "platform_monitor_agent is DORMANT. Set is_active=true in Config Registry to activate."
        )
    # engine = resolve_engine("basco", "reputation-management")
    # return LlmAgent(
    #     name="platform_monitor_agent",
    #     model=engine["model"],
    #     instruction=open("prompts/platform-monitor.md").read(),
    # )
