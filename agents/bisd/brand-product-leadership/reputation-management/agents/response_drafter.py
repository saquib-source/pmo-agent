...
def build_agent():
    ...
    if not IS_ACTIVE:
        raise RuntimeError(
            "response_drafter_agent is DORMANT. Set is_active=true in Config Registry to activate."
        )
    # engine = resolve_engine("basco", "reputation-management")
    # return LlmAgent(
    #     name="response_drafter_agent",
    #     model=engine["model"],
    #     instruction=open("prompts/response-drafter.md").read(),
    # )
