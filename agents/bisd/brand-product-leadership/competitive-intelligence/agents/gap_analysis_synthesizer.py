"""
Gap Analysis Synthesizer
BISD swarm: competitive-intelligence
...
def build_agent():
    ...
    if not IS_ACTIVE:
        raise RuntimeError(
            "gap_analysis_synthesizer_agent is DORMANT. Set is_active=true in Config Registry to activate."
        )
    # engine = resolve_engine("basco", "competitive-intelligence")
    # return LlmAgent(
    #     name="gap_analysis_synthesizer_agent",
    #     model=engine["model"],
    #     instruction=open("prompts/gap-analysis-synthesizer.md").read(),
    # )
...
def run_stub(payload: dict) -> dict:
    ...
    result = {
        "agent": "gap_analysis_synthesizer_agent",
        "status": "NOOP",
        "reason": "STUB — agent is_active=false",
        "payload_received": bool(payload),
    }
    # log_decision(agent_id="competitive-intelligence", action="stub_noop", outcome="noop")
    return result
