...
def build_agent():
    ...
    if not IS_ACTIVE:
        raise RuntimeError(
            "spec_language_monitor_agent is DORMANT. Set is_active=true in Config Registry to activate."
        )
    # engine = resolve_engine("basco", "competitive-intelligence")
    # return LlmAgent(
    #     name="spec_language_monitor_agent",
    #     model=engine["model"],
    #     instruction=open("prompts/spec-language-monitor.md").read(),
    # )
...
def run_stub(payload: dict) -> dict:
    ...
    result = {
        "agent": "spec_language_monitor_agent",
        "status": "NOOP",
