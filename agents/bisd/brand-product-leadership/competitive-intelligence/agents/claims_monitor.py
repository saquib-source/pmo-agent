...
def build_agent():
    ...
    if not IS_ACTIVE:
        raise RuntimeError(
            "claims_monitor_agent is DORMANT. Set is_active=true in Config Registry to activate."
        )
    # engine = resolve_engine("basco", "competitive-intelligence")
    # return LlmAgent(
    #     name="claims_monitor_agent",
    #     model=engine["model"],
    #     instruction=open("prompts/claims-monitor.md").read(),
    # )
