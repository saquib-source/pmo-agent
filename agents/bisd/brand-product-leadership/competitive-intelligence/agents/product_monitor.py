...
def build_agent():
    ...
    if not IS_ACTIVE:
        raise RuntimeError(
            "product_monitor_agent is DORMANT. Set is_active=true in Config Registry to activate."
        )
    # engine = resolve_engine("basco", "competitive-intelligence")
    # return LlmAgent(
    #     name="product_monitor_agent",
    #     model=engine["model"],
    #     instruction=open("prompts/product-monitor.md").read(),
    # )
