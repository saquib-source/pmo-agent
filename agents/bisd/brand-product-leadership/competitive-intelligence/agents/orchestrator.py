...
def build_agent():
    ...
    if not IS_ACTIVE:
        raise RuntimeError(
            "competitive_intelligence_orchestrator is DORMANT. Set is_active=true in Config Registry to activate."
        )
    # engine = resolve_engine("basco", "competitive-intelligence")
    # return LlmAgent(
    #     name="competitive_intelligence_orchestrator",
    #     model=engine["model"],
...
def run_stub(payload: dict) -> dict:
    ...
    result = {
        "agent": "competitive_intelligence_orchestrator",
        "status": "NOOP",
