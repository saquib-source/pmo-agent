"""
Equivalence Proof Orchestrator
BISD swarm: equivalence-proof
...
def build_agent():
    ...
    if not IS_ACTIVE:
        raise RuntimeError(
            "equivalence_proof_orchestrator is DORMANT. Set is_active=true in Config Registry to activate."
        )
    # engine = resolve_engine("basco", "equivalence-proof")
    # return LlmAgent(
    #     name="equivalence_proof_orchestrator",
    #     model=engine["model"],
...
def run_stub(payload: dict) -> dict:
    ...
    result = {
        "agent": "equivalence_proof_orchestrator",
        "status": "NOOP",
        "reason": "STUB — agent is_active=false",
        "payload_received": bool(payload),
    }
    # log_decision(agent_id="equivalence-proof", action="stub_noop", outcome="noop")
    return result
