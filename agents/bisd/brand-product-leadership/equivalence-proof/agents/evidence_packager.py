...
def build_agent():
    ...
    if not IS_ACTIVE:
        raise RuntimeError(
            "evidence_packager_agent is DORMANT. Set is_active=true in Config Registry to activate."
        )
    # engine = resolve_engine("basco", "equivalence-proof")
    # return LlmAgent(
    #     name="evidence_packager_agent",
    #     model=engine["model"],
    #     instruction=open("prompts/evidence-packager.md").read(),
    # )
