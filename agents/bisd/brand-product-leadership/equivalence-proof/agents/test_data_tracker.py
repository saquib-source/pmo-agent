...
def build_agent():
    ...
    if not IS_ACTIVE:
        raise RuntimeError(
            "test_data_tracker_agent is DORMANT. Set is_active=true in Config Registry to activate."
        )
    # engine = resolve_engine("basco", "equivalence-proof")
    # return LlmAgent(
    #     name="test_data_tracker_agent",
    #     model=engine["model"],
    #     instruction=open("prompts/test-data-tracker.md").read(),
    # )
