"""
Early Project Discovery Orchestrator
...
def build_agent():
    ...
    if not IS_ACTIVE:
        raise RuntimeError(
            "early_project_discovery_orchestrator is DORMANT. Set is_active=true in Config Registry to activate."
        )
    # engine = resolve_engine("basco", "early-project-discovery")
    # return LlmAgent(
    #     name="early_project_discovery_orchestrator",
    #     model=engine["model"],
...
def run_stub(payload: dict) -> dict:
    ...
    result = {
        "agent": "early_project_discovery_orchestrator",
        "status": "NOOP",
