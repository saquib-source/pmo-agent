...
def build_agent():
    ...
    if not IS_ACTIVE:
        raise RuntimeError(
            "zoning_monitor_agent is DORMANT. Set is_active=true in Config Registry to activate."
        )
    # engine = resolve_engine("basco", "early-project-discovery")
    # return LlmAgent(
    #     name="zoning_monitor_agent",
    #     model=engine["model"],
    #     instruction=open("prompts/zoning-monitor.md").read(),
    # )
