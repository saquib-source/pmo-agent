...
def build_agent():
    ...
    if not IS_ACTIVE:
        raise RuntimeError(
            "relevance_scorer_agent is DORMANT. Set is_active=true in Config Registry to activate."
        )
    # engine = resolve_engine("basco", "publicized-project-aggregation")
    # return LlmAgent(
    #     name="relevance_scorer_agent",
    #     model=engine["model"],
    #     instruction=open("prompts/relevance-scorer.md").read(),
    # )
