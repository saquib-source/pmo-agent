...
def build_agent():
    ...
    if not IS_ACTIVE:
        raise RuntimeError(
            "dedup_agent_agent is DORMANT. Set is_active=true in Config Registry to activate."
        )
    # engine = resolve_engine("basco", "publicized-project-aggregation")
    # return LlmAgent(
    #     name="dedup_agent_agent",
    #     model=engine["model"],
    #     instruction=open("prompts/dedup-agent.md").read(),
    # )
