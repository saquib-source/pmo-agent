...
def build_agent():
    ...
    if not IS_ACTIVE:
        raise RuntimeError(
            "newsletter_parser_agent is DORMANT. Set is_active=true in Config Registry to activate."
        )
    # engine = resolve_engine("basco", "publicized-project-aggregation")
    # return LlmAgent(
    #     name="newsletter_parser_agent",
    #     model=engine["model"],
    #     instruction=open("prompts/newsletter-parser.md").read(),
    # )
