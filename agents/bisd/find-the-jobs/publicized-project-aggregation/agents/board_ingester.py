...
def build_agent():
    ...
    if not IS_ACTIVE:
        raise RuntimeError(
            "board_ingester_agent is DORMANT. Set is_active=true in Config Registry to activate."
        )
    # engine = resolve_engine("basco", "publicized-project-aggregation")
    # return LlmAgent(
    #     name="board_ingester_agent",
    #     model=engine["model"],
    #     instruction=open("prompts/board-ingester.md").read(),
    # )
