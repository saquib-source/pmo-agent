"""
Publicized Project Aggregation Orchestrator
BISD swarm: publicized-project-aggregation
...
def build_agent():
    ...
    if not IS_ACTIVE:
        raise RuntimeError(
            "publicized_project_aggregation_orchestrator is DORMANT. Set is_active=true in Config Registry to activate."
        )
    # engine = resolve_engine("basco", "publicized-project-aggregation")
    # return LlmAgent(
    #     name="publicized_project_aggregation_orchestrator",
    #     model=engine["model"],
...
def run_stub(payload: dict) -> dict:
    ...
    result = {
        "agent": "publicized_project_aggregation_orchestrator",
        "status": "NOOP",
        "reason": "STUB — agent is_active=false",
        "payload_received": bool(payload),
    }
    # log_decision(agent_id="publicized-project-aggregation", action="stub_noop", outcome="noop")
    return result
