...
def build_agent():
    ...
    if not IS_ACTIVE:
        raise RuntimeError(
            "recommendation_synthesizer_agent is DORMANT. Set is_active=true in Config Registry to activate."
        )
    # engine = resolve_engine("basco", "bid-selection")
    # return LlmAgent(
    #     name="recommendation_synthesizer_agent",
    #     model=engine["model"],
    #     instruction=open("prompts/recommendation-synthesizer.md").read(),
    # )
