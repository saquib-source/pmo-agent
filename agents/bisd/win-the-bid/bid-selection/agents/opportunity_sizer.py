...
def build_agent():
    ...
    if not IS_ACTIVE:
        raise RuntimeError(
            "opportunity_sizer_agent is DORMANT. Set is_active=true in Config Registry to activate."
        )
    # engine = resolve_engine("basco", "bid-selection")
    # return LlmAgent(
    #     name="opportunity_sizer_agent",
    #     model=engine["model"],
    #     instruction=open("prompts/opportunity-sizer.md").read(),
    # )
