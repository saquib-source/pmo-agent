...
def build_agent():
    ...
    if not IS_ACTIVE:
        raise RuntimeError(
            "rfp_parser_agent is DORMANT. Set is_active=true in Config Registry to activate."
        )
    # engine = resolve_engine("basco", "bid-selection")
    # return LlmAgent(
    #     name="rfp_parser_agent",
    #     model=engine["model"],
    #     instruction=open("prompts/rfp-parser.md").read(),
    # )
