...
def build_agent():
    ...
    if not IS_ACTIVE:
        raise RuntimeError(
            "substitution_detector_agent is DORMANT. Set is_active=true in Config Registry to activate."
        )
    # engine = resolve_engine("basco", "bid-selection")
    # return LlmAgent(
    #     name="substitution_detector_agent",
    #     model=engine["model"],
    #     instruction=open("prompts/substitution-detector.md").read(),
    # )
...
def run_stub(payload: dict) -> dict:
    ...
    result = {
        "agent": "substitution_detector_agent",
        "status": "NOOP",
