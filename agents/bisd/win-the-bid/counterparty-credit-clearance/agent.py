"""
Counterparty Credit Clearance
BISD swarm: counterparty-credit-clearance
...
def build_agent():
    ...
    if not IS_ACTIVE:
        raise RuntimeError(
            "counterparty_credit_clearance_agent is DORMANT. Set is_active=true in Config Registry to activate."
        )
    # engine = resolve_engine("basco", "counterparty-credit-clearance")
    # return LlmAgent(
    #     name="counterparty_credit_clearance_agent",
    #     model=engine["model"],
...
def run_stub(payload: dict) -> dict:
    ...
    result = {
        "agent": "counterparty_credit_clearance_agent",
        "status": "NOOP",
        "reason": "STUB — agent is_active=false",
        "payload_received": bool(payload),
    }
    # log_decision(agent_id="counterparty-credit-clearance", action="stub_noop", outcome="noop")
    return result
