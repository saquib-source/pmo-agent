"""
Vendor Account Opener
BISD swarm: division-10-sourcing
...
def build_agent():
    ...
    if not IS_ACTIVE:
        raise RuntimeError(
            "vendor_account_opener_agent is DORMANT. Set is_active=true in Config Registry to activate."
        )
    # engine = resolve_engine("basco", "division-10-sourcing")
    # return LlmAgent(
    #     name="vendor_account_opener_agent",
    #     model=engine["model"],
    #     instruction=open("prompts/vendor-account-opener.md").read(),
    # )
...
def run_stub(payload: dict) -> dict:
    ...
    result = {
        "agent": "vendor_account_opener_agent",
        "status": "NOOP",
        "reason": "STUB — agent is_active=false",
        "payload_received": bool(payload),
    }
    # log_decision(agent_id="division-10-sourcing", action="stub_noop", outcome="noop")
    return result
