...
def build_agent():
    ...
    if not IS_ACTIVE:
        raise RuntimeError(
            "vendor_pricer_agent is DORMANT. Set is_active=true in Config Registry to activate."
        )
    # engine = resolve_engine("basco", "division-10-sourcing")
    # return LlmAgent(
    #     name="vendor_pricer_agent",
    #     model=engine["model"],
    #     instruction=open("prompts/vendor-pricer.md").read(),
    # )
