"""
Division 10 Sourcing Orchestrator
...
def build_agent():
    ...
    if not IS_ACTIVE:
        raise RuntimeError(
            "division_10_sourcing_orchestrator is DORMANT. Set is_active=true in Config Registry to activate."
        )
    # engine = resolve_engine("basco", "division-10-sourcing")
    # return LlmAgent(
    #     name="division_10_sourcing_orchestrator",
    #     model=engine["model"],
...
def run_stub(payload: dict) -> dict:
    ...
    result = {
        "agent": "division_10_sourcing_orchestrator",
        "status": "NOOP",
