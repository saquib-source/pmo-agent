...
def build_agent():
    ...
    if not IS_ACTIVE:
        raise RuntimeError(
            "sample_fulfillment_agent is DORMANT. Set is_active=true in Config Registry to activate."
        )
    # engine = resolve_engine("basco", "substitution-approval")
    # return LlmAgent(
    #     name="sample_fulfillment_agent",
    #     model=engine["model"],
    #     instruction=open("prompts/sample-fulfillment.md").read(),
    # )
...
def run_stub(payload: dict) -> dict:
    ...
    result = {
        "agent": "sample_fulfillment_agent",
        "status": "NOOP",
