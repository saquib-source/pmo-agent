"""
Substitution Approval Orchestrator
BISD swarm: substitution-approval
...
def build_agent():
    ...
    if not IS_ACTIVE:
        raise RuntimeError(
            "substitution_approval_orchestrator is DORMANT. Set is_active=true in Config Registry to activate."
        )
    # engine = resolve_engine("basco", "substitution-approval")
    # return LlmAgent(
    #     name="substitution_approval_orchestrator",
    #     model=engine["model"],
...
def run_stub(payload: dict) -> dict:
    ...
    result = {
        "agent": "substitution_approval_orchestrator",
        "status": "NOOP",
        "reason": "STUB — agent is_active=false",
        "payload_received": bool(payload),
    }
    # log_decision(agent_id="substitution-approval", action="stub_noop", outcome="noop")
    return result
