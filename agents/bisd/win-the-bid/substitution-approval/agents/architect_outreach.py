...
def build_agent():
    ...
    if not IS_ACTIVE:
        raise RuntimeError(
            "architect_outreach_agent is DORMANT. Set is_active=true in Config Registry to activate."
        )
    # engine = resolve_engine("basco", "substitution-approval")
    # return LlmAgent(
    #     name="architect_outreach_agent",
    #     model=engine["model"],
    #     instruction=open("prompts/architect-outreach.md").read(),
    # )
...
def run_stub(payload: dict) -> dict:
    ...
    result = {
        "agent": "architect_outreach_agent",
        "status": "NOOP",
