...
def build_agent():
    ...
    if not IS_ACTIVE:
        raise RuntimeError(
            "addendum_tracker_agent is DORMANT. Set is_active=true in Config Registry to activate."
        )
    # engine = resolve_engine("basco", "substitution-approval")
    # return LlmAgent(
    #     name="addendum_tracker_agent",
    #     model=engine["model"],
    #     instruction=open("prompts/addendum-tracker.md").read(),
    # )
