...
def build_agent():
    ...
    if not IS_ACTIVE:
        raise RuntimeError(
            "margin_calculator_agent is DORMANT. Set is_active=true in Config Registry to activate."
        )
    # engine = resolve_engine("basco", "bid-pricing")
    # return LlmAgent(
    #     name="margin_calculator_agent",
    #     model=engine["model"],
    #     instruction=open("prompts/margin-calculator.md").read(),
    # )
