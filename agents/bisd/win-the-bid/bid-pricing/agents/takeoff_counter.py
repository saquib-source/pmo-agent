...
def build_agent():
    ...
    if not IS_ACTIVE:
        raise RuntimeError(
            "takeoff_counter_agent is DORMANT. Set is_active=true in Config Registry to activate."
        )
    # engine = resolve_engine("basco", "bid-pricing")
    # return LlmAgent(
    #     name="takeoff_counter_agent",
    #     model=engine["model"],
    #     instruction=open("prompts/takeoff-counter.md").read(),
    # )
