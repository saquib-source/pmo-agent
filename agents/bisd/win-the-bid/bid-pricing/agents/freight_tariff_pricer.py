"""
Freight and Tariff Pricer
BISD swarm: bid-pricing
...
def build_agent():
    ...
    if not IS_ACTIVE:
        raise RuntimeError(
            "freight_tariff_pricer_agent is DORMANT. Set is_active=true in Config Registry to activate."
        )
    # engine = resolve_engine("basco", "bid-pricing")
    # return LlmAgent(
    #     name="freight_tariff_pricer_agent",
    #     model=engine["model"],
    #     instruction=open("prompts/freight-tariff-pricer.md").read(),
    # )
...
def run_stub(payload: dict) -> dict:
    ...
    result = {
        "agent": "freight_tariff_pricer_agent",
        "status": "NOOP",
        "reason": "STUB — agent is_active=false",
        "payload_received": bool(payload),
    }
    # log_decision(agent_id="bid-pricing", action="stub_noop", outcome="noop")
    return result
