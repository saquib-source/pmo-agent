...
def build_agent():
    ...
    if not IS_ACTIVE:
        raise RuntimeError(
            "bid_pricing_orchestrator is DORMANT. Set is_active=true in Config Registry to activate."
        )
    # engine = resolve_engine("basco", "bid-pricing")
    # return LlmAgent(
    #     name="bid_pricing_orchestrator",
    #     model=engine["model"],
...
def run_stub(payload: dict) -> dict:
    ...
    result = {
        "agent": "bid_pricing_orchestrator",
        "status": "NOOP",
