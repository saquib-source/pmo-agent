...
def build_agent():
    ...
    if not IS_ACTIVE:
        raise RuntimeError(
            "bid_submission_agent is DORMANT. Set is_active=true in Config Registry to activate."
        )
    # engine = resolve_engine("basco", "bid-submission")
    # return LlmAgent(
    #     name="bid_submission_agent",
    #     model=engine["model"],
...
def run_stub(payload: dict) -> dict:
    ...
    result = {
        "agent": "bid_submission_agent",
        "status": "NOOP",
        "reason": "STUB — agent is_active=false",
        "payload_received": bool(payload),
    }
    # log_decision(agent_id="bid-submission", action="stub_noop", outcome="noop")
    return result
