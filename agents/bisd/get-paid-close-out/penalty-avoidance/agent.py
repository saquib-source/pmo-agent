...
def build_agent():
    ...
    if not IS_ACTIVE:
        raise RuntimeError(
            "penalty_avoidance_agent is DORMANT. Set is_active=true in Config Registry to activate."
        )
    # engine = resolve_engine("basco", "penalty-avoidance")
    # return LlmAgent(
    #     name="penalty_avoidance_agent",
    #     model=engine["model"],
...
def run_stub(payload: dict) -> dict:
    ...
    result = {
        "agent": "penalty_avoidance_agent",
        "status": "NOOP",
        "reason": "STUB — agent is_active=false",
        "payload_received": bool(payload),
    }
    # log_decision(agent_id="penalty-avoidance", action="stub_noop", outcome="noop")
    return result
