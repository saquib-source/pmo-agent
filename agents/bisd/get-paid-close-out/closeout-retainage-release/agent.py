...
BISD swarm: closeout-retainage-release
...
def build_agent():
    ...
    if not IS_ACTIVE:
        raise RuntimeError(
            "closeout_retainage_release_agent is DORMANT. Set is_active=true in Config Registry to activate."
        )
    # engine = resolve_engine("basco", "closeout-retainage-release")
    # return LlmAgent(
    #     name="closeout_retainage_release_agent",
    #     model=engine["model"],
...
def run_stub(payload: dict) -> dict:
    ...
    result = {
        "agent": "closeout_retainage_release_agent",
        "status": "NOOP",
        "reason": "STUB — agent is_active=false",
        "payload_received": bool(payload),
    }
    # log_decision(agent_id="closeout-retainage-release", action="stub_noop", outcome="noop")
    return result
