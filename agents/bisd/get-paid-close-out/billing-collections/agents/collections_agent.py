...
def build_agent():
    ...
    if not IS_ACTIVE:
        raise RuntimeError(
            "collections_agent_agent is DORMANT. Set is_active=true in Config Registry to activate."
        )
    # engine = resolve_engine("basco", "billing-collections")
    # return LlmAgent(
    #     name="collections_agent_agent",
    #     model=engine["model"],
    #     instruction=open("prompts/collections-agent.md").read(),
    # )
