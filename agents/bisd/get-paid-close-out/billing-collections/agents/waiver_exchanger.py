...
def build_agent():
    ...
    if not IS_ACTIVE:
        raise RuntimeError(
            "waiver_exchanger_agent is DORMANT. Set is_active=true in Config Registry to activate."
        )
    # engine = resolve_engine("basco", "billing-collections")
    # return LlmAgent(
    #     name="waiver_exchanger_agent",
    #     model=engine["model"],
    #     instruction=open("prompts/waiver-exchanger.md").read(),
    # )
