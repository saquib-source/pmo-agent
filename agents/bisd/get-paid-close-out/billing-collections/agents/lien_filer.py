...
def build_agent():
    ...
    if not IS_ACTIVE:
        raise RuntimeError(
            "lien_filer_agent is DORMANT. Set is_active=true in Config Registry to activate."
        )
    # engine = resolve_engine("basco", "billing-collections")
    # return LlmAgent(
    #     name="lien_filer_agent",
    #     model=engine["model"],
    #     instruction=open("prompts/lien-filer.md").read(),
    # )
