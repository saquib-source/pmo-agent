"""
Billing and Collections Orchestrator
BISD swarm: billing-collections
...
def build_agent():
    ...
    if not IS_ACTIVE:
        raise RuntimeError(
            "billing_collections_orchestrator is DORMANT. Set is_active=true in Config Registry to activate."
        )
    # engine = resolve_engine("basco", "billing-collections")
    # return LlmAgent(
    #     name="billing_collections_orchestrator",
    #     model=engine["model"],
...
def run_stub(payload: dict) -> dict:
    ...
    result = {
        "agent": "billing_collections_orchestrator",
        "status": "NOOP",
        "reason": "STUB — agent is_active=false",
        "payload_received": bool(payload),
    }
    # log_decision(agent_id="billing-collections", action="stub_noop", outcome="noop")
    return result
