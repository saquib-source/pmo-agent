...
def build_agent():
    ...
    if not IS_ACTIVE:
        raise RuntimeError(
            "progress_invoicer_agent is DORMANT. Set is_active=true in Config Registry to activate."
        )
    # engine = resolve_engine("basco", "billing-collections")
    # return LlmAgent(
    #     name="progress_invoicer_agent",
    #     model=engine["model"],
    #     instruction=open("prompts/progress-invoicer.md").read(),
    # )
