...
def build_agent():
    ...
    if not IS_ACTIVE:
        raise RuntimeError(
            "installer_payment_agent is DORMANT. Set is_active=true in Config Registry to activate."
        )
    # engine = resolve_engine("basco", "installer-payment")
    # return LlmAgent(
    #     name="installer_payment_agent",
    #     model=engine["model"],
...
def run_stub(payload: dict) -> dict:
    ...
    result = {
        "agent": "installer_payment_agent",
        "status": "NOOP",
        "reason": "STUB — agent is_active=false",
        "payload_received": bool(payload),
    }
    # log_decision(agent_id="installer-payment", action="stub_noop", outcome="noop")
    return result
