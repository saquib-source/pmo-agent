...
BISD swarm: insurance-compliance
...
def build_agent():
    ...
    if not IS_ACTIVE:
        raise RuntimeError(
            "insurance_compliance_agent is DORMANT. Set is_active=true in Config Registry to activate."
        )
    # engine = resolve_engine("basco", "insurance-compliance")
    # return LlmAgent(
...
def run_stub(payload: dict) -> dict:
    ...
    result = {
        "agent": "insurance_compliance_agent",
        "status": "NOOP",
        "reason": "STUB — agent is_active=false",
        "payload_received": bool(payload),
    }
    # log_decision(agent_id="insurance-compliance", action="stub_noop", outcome="noop")
    return result
