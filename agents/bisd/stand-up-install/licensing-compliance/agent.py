...
BISD swarm: licensing-compliance
...
def build_agent():
    ...
    if not IS_ACTIVE:
        raise RuntimeError(
            "licensing_compliance_agent is DORMANT. Set is_active=true in Config Registry to activate."
        )
    # engine = resolve_engine("basco", "licensing-compliance")
    # return LlmAgent(
    #     name="licensing_compliance_agent",
    #     model=engine["model"],
...
def run_stub(payload: dict) -> dict:
    ...
    result = {
        "agent": "licensing_compliance_agent",
        "status": "NOOP",
        "reason": "STUB — agent is_active=false",
        "payload_received": bool(payload),
    }
    # log_decision(agent_id="licensing-compliance", action="stub_noop", outcome="noop")
    return result
