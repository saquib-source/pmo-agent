...
def build_agent():
    ...
    if not IS_ACTIVE:
        raise RuntimeError(
            "labor_pricing_agent is DORMANT. Set is_active=true in Config Registry to activate."
        )
    # engine = resolve_engine("basco", "labor-pricing")
    # return LlmAgent(
    #     name="labor_pricing_agent",
    #     model=engine["model"],
...
def run_stub(payload: dict) -> dict:
    ...
    result = {
        "agent": "labor_pricing_agent",
        "status": "NOOP",
        "reason": "STUB — agent is_active=false",
        "payload_received": bool(payload),
    }
    # log_decision(agent_id="labor-pricing", action="stub_noop", outcome="noop")
    return result
