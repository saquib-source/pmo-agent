...
def build_agent():
    ...
    if not IS_ACTIVE:
        raise RuntimeError(
            "coverage_mapper_agent is DORMANT. Set is_active=true in Config Registry to activate."
        )
    # engine = resolve_engine("basco", "installer-coverage")
    # return LlmAgent(
    #     name="coverage_mapper_agent",
    #     model=engine["model"],
    #     instruction=open("prompts/coverage-mapper.md").read(),
    # )
