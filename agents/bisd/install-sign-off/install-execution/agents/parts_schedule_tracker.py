"""
Parts Schedule Tracker
BISD swarm: install-execution
...
def build_agent():
    ...
    if not IS_ACTIVE:
        raise RuntimeError(
            "parts_schedule_tracker_agent is DORMANT. Set is_active=true in Config Registry to activate."
        )
    # engine = resolve_engine("basco", "install-execution")
    # return LlmAgent(
    #     name="parts_schedule_tracker_agent",
    #     model=engine["model"],
    #     instruction=open("prompts/parts-schedule-tracker.md").read(),
    # )
...
def run_stub(payload: dict) -> dict:
    ...
    result = {
        "agent": "parts_schedule_tracker_agent",
        "status": "NOOP",
        "reason": "STUB — agent is_active=false",
        "payload_received": bool(payload),
    }
    # log_decision(agent_id="install-execution", action="stub_noop", outcome="noop")
    return result
