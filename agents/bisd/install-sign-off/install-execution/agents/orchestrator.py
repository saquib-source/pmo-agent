"""
Install Execution Orchestrator
...
def build_agent():
    ...
    if not IS_ACTIVE:
        raise RuntimeError(
            "install_execution_orchestrator is DORMANT. Set is_active=true in Config Registry to activate."
        )
    # engine = resolve_engine("basco", "install-execution")
    # return LlmAgent(
    #     name="install_execution_orchestrator",
    #     model=engine["model"],
...
def run_stub(payload: dict) -> dict:
    ...
    result = {
        "agent": "install_execution_orchestrator",
        "status": "NOOP",
