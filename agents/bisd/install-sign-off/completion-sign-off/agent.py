"""
Completion Sign-Off
BISD swarm: completion-sign-off
...
def build_agent():
    ...
    if not IS_ACTIVE:
        raise RuntimeError(
            "completion_sign_off_agent is DORMANT. Set is_active=true in Config Registry to activate."
        )
    # engine = resolve_engine("basco", "completion-sign-off")
    # return LlmAgent(
    #     name="completion_sign_off_agent",
    #     model=engine["model"],
...
def run_stub(payload: dict) -> dict:
    ...
    result = {
        "agent": "completion_sign_off_agent",
        "status": "NOOP",
        "reason": "STUB — agent is_active=false",
        "payload_received": bool(payload),
    }
    # log_decision(agent_id="completion-sign-off", action="stub_noop", outcome="noop")
    return result
