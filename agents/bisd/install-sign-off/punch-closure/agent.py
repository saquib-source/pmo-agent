...
def build_agent():
    ...
    if not IS_ACTIVE:
        raise RuntimeError(
            "punch_closure_agent is DORMANT. Set is_active=true in Config Registry to activate."
        )
    # engine = resolve_engine("basco", "punch-closure")
    # return LlmAgent(
    #     name="punch_closure_agent",
    #     model=engine["model"],
...
def run_stub(payload: dict) -> dict:
    ...
    result = {
        "agent": "punch_closure_agent",
        "status": "NOOP",
        "reason": "STUB — agent is_active=false",
        "payload_received": bool(payload),
    }
    # log_decision(agent_id="punch-closure", action="stub_noop", outcome="noop")
    return result
