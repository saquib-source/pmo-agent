...
def build_agent():
    ...
    if not IS_ACTIVE:
        raise RuntimeError(
            "pipeline_aggregator_agent is DORMANT. Set is_active=true in Config Registry to activate."
        )
    # engine = resolve_engine("basco", "pipeline-margin-oversight")
    # return LlmAgent(
    #     name="pipeline_aggregator_agent",
    #     model=engine["model"],
    #     instruction=open("prompts/pipeline-aggregator.md").read(),
    # )
...
def run_stub(payload: dict) -> dict:
    ...
    result = {
        "agent": "pipeline_aggregator_agent",
        "status": "NOOP",
