...
def build_agent():
    ...
    if not IS_ACTIVE:
        raise RuntimeError(
            "win_loss_analyzer_agent is DORMANT. Set is_active=true in Config Registry to activate."
        )
    # engine = resolve_engine("basco", "pipeline-margin-oversight")
    # return LlmAgent(
    #     name="win_loss_analyzer_agent",
    #     model=engine["model"],
    #     instruction=open("prompts/win-loss-analyzer.md").read(),
    # )
