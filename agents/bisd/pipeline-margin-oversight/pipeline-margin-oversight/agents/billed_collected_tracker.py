"""
Billed-vs-Collected Tracker
BISD swarm: pipeline-margin-oversight
...
def build_agent():
    ...
    if not IS_ACTIVE:
        raise RuntimeError(
            "billed_collected_tracker_agent is DORMANT. Set is_active=true in Config Registry to activate."
        )
    # engine = resolve_engine("basco", "pipeline-margin-oversight")
    # return LlmAgent(
    #     name="billed_collected_tracker_agent",
    #     model=engine["model"],
    #     instruction=open("prompts/billed-collected-tracker.md").read(),
    # )
...
def run_stub(payload: dict) -> dict:
    ...
    result = {
        "agent": "billed_collected_tracker_agent",
        "status": "NOOP",
        "reason": "STUB — agent is_active=false",
        "payload_received": bool(payload),
    }
    # log_decision(agent_id="pipeline-margin-oversight", action="stub_noop", outcome="noop")
    return result
