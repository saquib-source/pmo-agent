...
def build_agent():
    ...
    if not IS_ACTIVE:
        raise RuntimeError(
            "margin_by_mix_calculator_agent is DORMANT. Set is_active=true in Config Registry to activate."
        )
    # engine = resolve_engine("basco", "pipeline-margin-oversight")
    # return LlmAgent(
    #     name="margin_by_mix_calculator_agent",
    #     model=engine["model"],
    #     instruction=open("prompts/margin-by-mix-calculator.md").read(),
    # )
...
def run_stub(payload: dict) -> dict:
    ...
    result = {
        "agent": "margin_by_mix_calculator_agent",
        "status": "NOOP",
