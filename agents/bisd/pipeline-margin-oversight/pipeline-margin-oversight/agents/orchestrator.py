"""
Pipeline and Margin Oversight Orchestrator
...
def build_agent():
    ...
    if not IS_ACTIVE:
        raise RuntimeError(
            "pipeline_margin_oversight_orchestrator is DORMANT. Set is_active=true in Config Registry to activate."
        )
    # engine = resolve_engine("basco", "pipeline-margin-oversight")
    # return LlmAgent(
    #     name="pipeline_margin_oversight_orchestrator",
    #     model=engine["model"],
...
def run_stub(payload: dict) -> dict:
    ...
    result = {
        "agent": "pipeline_margin_oversight_orchestrator",
        "status": "NOOP",
