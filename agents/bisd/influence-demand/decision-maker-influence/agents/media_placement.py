...
def build_agent():
    ...
    if not IS_ACTIVE:
        raise RuntimeError(
            "media_placement_agent is DORMANT. Set is_active=true in Config Registry to activate."
        )
    # engine = resolve_engine("basco", "decision-maker-influence")
    # return LlmAgent(
    #     name="media_placement_agent",
    #     model=engine["model"],
    #     instruction=open("prompts/media-placement.md").read(),
    # )
