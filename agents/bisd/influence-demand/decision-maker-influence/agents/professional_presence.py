...
def build_agent():
    ...
    if not IS_ACTIVE:
        raise RuntimeError(
            "professional_presence_agent is DORMANT. Set is_active=true in Config Registry to activate."
        )
    # engine = resolve_engine("basco", "decision-maker-influence")
    # return LlmAgent(
    #     name="professional_presence_agent",
    #     model=engine["model"],
    #     instruction=open("prompts/professional-presence.md").read(),
    # )
...
def run_stub(payload: dict) -> dict:
    ...
    result = {
        "agent": "professional_presence_agent",
        "status": "NOOP",
