"""
Direct-Contact Sequencer
BISD swarm: decision-maker-influence
...
def build_agent():
    ...
    if not IS_ACTIVE:
        raise RuntimeError(
            "direct_contact_sequencer_agent is DORMANT. Set is_active=true in Config Registry to activate."
        )
    # engine = resolve_engine("basco", "decision-maker-influence")
    # return LlmAgent(
    #     name="direct_contact_sequencer_agent",
    #     model=engine["model"],
    #     instruction=open("prompts/direct-contact-sequencer.md").read(),
    # )
...
def run_stub(payload: dict) -> dict:
    ...
    result = {
        "agent": "direct_contact_sequencer_agent",
        "status": "NOOP",
        "reason": "STUB — agent is_active=false",
        "payload_received": bool(payload),
    }
    # log_decision(agent_id="decision-maker-influence", action="stub_noop", outcome="noop")
    return result
