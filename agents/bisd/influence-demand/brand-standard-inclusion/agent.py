"""
Brand-Standard Inclusion
BISD swarm: brand-standard-inclusion
...
def build_agent():
    ...
    if not IS_ACTIVE:
        raise RuntimeError(
            "brand_standard_inclusion_agent is DORMANT. Set is_active=true in Config Registry to activate."
        )
    # engine = resolve_engine("basco", "brand-standard-inclusion")
    # return LlmAgent(
    #     name="brand_standard_inclusion_agent",
    #     model=engine["model"],
...
def run_stub(payload: dict) -> dict:
    ...
    result = {
        "agent": "brand_standard_inclusion_agent",
        "status": "NOOP",
        "reason": "STUB — agent is_active=false",
        "payload_received": bool(payload),
    }
    # log_decision(agent_id="brand-standard-inclusion", action="stub_noop", outcome="noop")
    return result
